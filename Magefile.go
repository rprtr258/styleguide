...
var PWD = os.getwd()
var BINDIR = filepath.join(PWD, "bin")

const APP_NAME = "myapp"

var RED = exec("tput", "-Txterm", "setaf", "1")
var GREEN = exec("tput", "-Txterm", "setaf", "2")
var YELLOW = exec("tput", "-Txterm", "setaf", "3")
var BLUE = exec("tput", "-Txterm", "setaf", "4")
var VIOLET = exec("tput", "-Txterm", "setaf", "5")
var CYAN = exec("tput", "-Txterm", "setaf", "6")
var WHITE = exec("tput", "-Txterm", "setaf", "7")
var RESET = exec("tput", "-Txterm", "sgr0")

const GOVULNCHECK_VERSION = "latest"
const GOLANGCILINT_VERSION = "v1.50"
const PROTOLINT_VERSION = "v0.40.0"
const MOCKGEN_VERSION = "v1.6.0"
const PROTOC_GEN_GO_VERSION = "v1.28"
const PROTOC_GEN_GO_GRPC_VERSION = "v1.2"
const PROTOC_VERSION = "3.19.0"
const NANCY_VERSION = "latest"
const GOMODOUTDATED_VERSION = "latest"
const GOLANGCILINTVER = "1.53.2"
...
func lint_go() {
	...
	if !file_exists {
		exec("wget", fmt.Sprintf("https://github.com/golangci/golangci-lint/releases/download/v%s/golangci-lint-%s-linux-amd64.tar.gz", GOLANGCILINTVER, GOLANGCILINTVER), "-O", GOLANGCILINTBIN+".tar.gz")
		exec("tar", "xvf", GOLANGCILINTBIN+".tar.gz", "-C", BINDIR)
		os.rename(filepath.join(BINDIR, fmt.Sprintf("golangci-lint-%s-linux-amd64/golangci-lint", GOLANGCILINTVER)), GOLANGCILINTBIN)
		os.remove(filepath.join(BINDIR, GOLANGCILINTBIN+".tar.gz"))
		os.remove_all(filepath.join(BINDIR, fmt.Sprintf("golangci-lint-%s-linux-amd64", GOLANGCILINTVER)))
	}
...
func gorun(tool, version, args ...string) {
